...
function fo_druidFilter(spellName)
    ...
    -- Use pcall to protect against runtime errors during talent resets
    -- or API state inconsistencies.
    local status, result = pcall(function()
        -- Guard: Ensure settings are loaded
        if not fo_Settings then return true end
    ...
    -- If pcall fails, return true (fail-safe) to avoid blocking actions during resets
